...
export class OpenViduLayout {
  ...
  private getBestDimensions(minR: number, maxR: number, count: number, WIDTH: number, HEIGHT: number, targetHeight: number) {
    let maxArea, targetCols, targetRows, targetWidth = 0, tWidth, tHeight, tRatio;
  ...
  private arrange(
    ...
    const count = children.length ? children.length : 0;
    let dimensions;
    ...
    if (!fixedRatio) {
      ...
      if(children.length > 0){
        const ratio = this.getVideoRatio(children[0]);
    ...
    for (let i = 0; i < children.length; i++) {
      if (dimensions && dimensions.targetCols && i % dimensions.targetCols === 0) {
        // This is a new row
        row = {
          children: [],
          width: 0,
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      for (let j = 0; j < row.children.length; j++) {
        ...
        let targetWidth = dimensions?.targetWidth ? dimensions.targetWidth : 0;
  ...
  initLayoutContainer(container:any, opts:any) {
    this.opts = {
